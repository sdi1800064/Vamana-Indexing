...
# EXCLUDE_FILES := $(SRC_DIR)/groundtruth.c $(SRC_DIR)/stitchedVamana.c
...
recallFilteredVamana: $(SRC_DIR)/dataset.c $(SRC_DIR)/graph.c $(SRC_DIR)/recallFilteredVamana.c
	$(CC) -o recallFilteredVamana $(SRC_DIR)/dataset.c $(SRC_DIR)/graph.c $(SRC_DIR)/recall.c $(CFLAGS)
...
clean:
	rm -rf $(OBJ_DIR)/*.o recall test groundtruth output.txt stitchedVamana
